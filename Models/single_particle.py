...
class SingleParticle(AbstractParticleSystem):
    ...
    def get_observables(self, H):
        ...
        #     self.x, self.y, self.z = np.mgrid[-H.extent / 2: H.extent / 2:H.N * 1j,
        #                              -H.extent / 2: H.extent / 2:H.N * 1j, -H.extent / 2: H.extent / 2:H.N * 1j]
        #     H.ndim = 3
    ...
    def compute_momentum_space(self, H):
        ...
        if H.spatial_ndim == 1:
            ...
            self.px = np.fft.fftshift(np.fft.fftfreq(H.N, d=H.dx)) * WP.hbar * 2 * np.pi
            self.py = np.fft.fftshift(np.fft.fftfreq(H.N, d=H.dx)) * WP.hbar * 2 * np.pi
            self.p2 = self.px ** 2
    ...
    def build_matrix_operators(self, H):
        ...
        #
        #     H.ndim = 3
        #     xx, yy, zz = np.mgrid[-H.extent / 2: H.extent / 2:H.N * 1j, -H.extent / 2: H.extent / 2:H.N * 1j,
    ...
    def get_kinetic_matrix(self, H):
        ...
        I = eye(H.N)
        T_ = diags([-2., 1., 1.], [0, -1, 1], shape=(H.N, H.N)) * -WP.k / (self.m * H.dx ** 2)
        if H.spatial_ndim == 1:
            T = T_
        #
        # elif H.spatial_ndim == 2:
    ...
    def get_eigenstates(self, H, max_states, eigenvalues, eigenvectors):
        ...
        eigenstates = Eigenstates(energies / WP.eV, eigenstates_array, H.extent, H.N, type)
        return eigenstates