...
class Hamiltonian:
    def __init__(self, particles, potential, N, extent, spatial_ndim=1, potential_type="grid", E_min=0):
        """
    ...
    def solve(self, max_states: int, method: str = 'eigsh', verbose=False,
              lobpcg_args={'N0': 30, 'preconditioner': 'jacobi', 'maxiter': 30}):
        """
        ...
        #                           spatial_ndim=3, N=lobpcg_args['N0'], extent=self.extent,
        #                           potential_type=self.potential_type, E_min=self.E_min)
        #
        #     eigenvalues_eigsh, eigenvectors_eigsh = eigsh(H_eigsh.V + H_eigsh.T, k=max_states, which='LM',
        #                                                   sigma=min(0, self.E_min))
        #
        ...
        #     new_xx, new_yy, new_zz, states = np.mgrid[-1:1:self.N * 1j, -1:1:self.N * 1j, -1:1:self.N * 1j,
        #                                      -1:1:max_states * 1j]
        #     eigenvectors_eigsh_interpolated = interpn((np.linspace(-1, 1, lobpcg_args['N0']),
        #                                                np.linspace(-1, 1, lobpcg_args['N0']),
        #                                                np.linspace(-1, 1, lobpcg_args['N0']),
        #                                                np.linspace(-1, 1, max_states)),
        #                                               eigenvectors_eigsh,
