...
class VisualizationSingleParticle1D(Visualization):
    ...
    def plot_eigenstate(self, k, xlim=None, show_imaginary_part=False):
        ...
        if show_imaginary_part == True:
            eigenstate_plot1 = ax1.plot(x / WP.Å, np.real(eigenstates_array[k]), label='$Re|\\psi(x)|$')
            eigenstate_plot2 = ax1.plot(x / WP.Å, np.imag(eigenstates_array[k]), label='$Im|\\psi(x)|$')
            eigenstate_plot3 = ax1.plot(x / WP.Å, np.abs(eigenstates_array[k]), label='$|\\psi(x)|$', color='white')
            ax1.legend()
        else:
            eigenstate_plot1 = ax1.plot(x / WP.Å, np.real(eigenstates_array[k]))
    ...
    def superpositions(self, states, fps=30, total_time=20, **kw):
        ...
        x = np.linspace(-self.eigenstates.extent / 2.0,
                        ...
                        len(eigenstates[0]))
        ax.set_yticks([])
        ax.set_xlim(np.array(params['xlim']) / WP.Å)

        line1, = ax.plot(x / WP.Å, np.real(eigenstates[0]), label='$Re|\\psi(x)|$')
        line2, = ax.plot(x / WP.Å, np.imag(eigenstates[0]), label='$Im|\\psi(x)|$')
        line3, = ax.plot(x / WP.Å, np.abs(eigenstates[0]), label='$|\\psi(x)|$', color='white')
...
#
#         potential_plot = ax.plot(x / Å, (self.simulation.H.Vgrid + self.simulation.Vmin) / (
#                     self.simulation.Vmax - self.simulation.Vmin), label='$V(x)$')
...
#         plt.show()
#
#     def animate(self, xlim=None, figsize=(16 / 9 * 5.804 * 0.9, 5.804), animation_duration=5, fps=20,
#                 save_animation=False):
